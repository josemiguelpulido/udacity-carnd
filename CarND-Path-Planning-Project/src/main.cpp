      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // if there is not enough points that have not been used yet
            // (previous path contains points sent to the simulator that have not been used yet)
            if (prev_size < 2) {
            ...
            // add evenly 30m spaced points (in frenet coordinates) ahead of the starting reference
            ...
            // transform to car's local coordinates to simplify math later on
            // (we also did it in a previous lesson for MPC)
            ...
            // start with the points left from the previous path (the ones not used yet)
            // (this is done in order to have a more smooth transition with the path already applied) 
            ...
            // calculate how to break up spline points we can travel at the desired reference velocity
            double target_x = 30.0; // arbitrary 30m ahead of car's current point
            double target_y = s(target_x);
            
            // compute distance from (0,0) to (target_x, target_y) using simple pythagoras
            // (recall we still are in car's coordinates)
            double target_dist = sqrt((target_x * target_x) + (target_y * target_y));
            
            double x_add_on = 0;
            
            // fill up the rest of the path planner after filling with previous points
            for (int i = 1; i < 50 - previous_path_x.size(); ++i) {
              ...
              // reverse car coordinates back into global(map) coordinates
